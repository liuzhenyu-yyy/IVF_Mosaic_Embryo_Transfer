...
for (one in Samples) {
   ...
   for (cell in cells) {
      p <- plotCNprofile(
         segref = SegRef[, cell], segcopy = SegCopy.filter[, cell],
...
plot(Venn(list(
   "GY" = cnv.manull$Cell[cnv.manull$GY == 1],
...
SegCopy_bulk[, ] <- 2
colnames(SegCopy_bulk) <- c(
   paste0("A", c(1:6, 8), "_CNV"),
   paste0("B", c(1:3), "_CNV")
)
...
for (one in 1:nrow(SegRef_bulk)) {
   SegRef_bulk[one, ] <- colMeans(SegRef_bulk.50bwa[countOverlaps(
      bin.info.50bwa.bed,
      bin.info.bed[one]
   ) > 0, ])
}
...
sample.info.CNV$Location <- as.numeric(lapply(
   apply(
      SegCopy.filter[, rownames(sample.info.CNV)] != 2,
      2,
      which
   ),
   min
...
for (one in levels(sample.info$Sample)) {
   ...
   ht1 <- Heatmap(t(SegCopy.filter.sex[, cells]),
      ...
      row_title_gp = gpar(
         fill = patient.color[sample.rename[one]],
         cex = 0.7
      ),
...
cnv.bin.list <- apply(SegCopy.filter[bin.auto, cells], 2, function(x) {
   return(which(x != 2))
})
