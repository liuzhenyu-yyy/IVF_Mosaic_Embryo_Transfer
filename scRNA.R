...
seu.obj <- CreateSeuratObject(rc,
  project = "fetal_genome",
  assay = "RNA",
...
seu.obj.qc <- NormalizeData(seu.obj.qc,
...
DimPlot(seu.obj.qc,
  reduction = "umap", group.by = "Sample_2",
  cols = patient.color
) +
...
FeaturePlot(seu.obj.qc,
  reduction = "umap", features = "prdict_frac",
  cols = c("gray", "red")
) +
  coord_equal(1.5) + theme_bw() +
  theme(panel.grid = element_blank())
...
seu.obj.cnv <- CreateSeuratObject(rc[, cell.select],
  project = "fetal_genome",
  assay = "RNA",
  meta.data = sample.info
)
...
seu.obj.cnv <- NormalizeData(seu.obj.cnv,
  normalization.method = "LogNormalize",
  scale.factor = 10000
)
...
marker.list.select <- c(
  "PTPRC", "CD3D", "CD3G", "NKG7", "KLRF1",
...
plist <- VlnPlot(seu.obj.cnv,
  features = marker.list.select,
...
for (one in 1:9) {
  sample.info.RNA$Cell <- gsub(
    paste0("_", one, "$"), paste0("_0", one),
    sample.info.RNA$Cell
  )
}
...
colnames(sample.info.RNA) <- c(
  "RNA_UMI_Number",
  "RNA_Gene_Number",
  "RNA_Mitochondrial_Ratio",
  "Pass_RNA_QC", "Cell_Type"
)
colnames(sample.info.DNA) <- c(
  "Patient",
  "Cell_ID",
